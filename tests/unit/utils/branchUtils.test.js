...
describe("branchUtils", () => {
  describe("matchesPattern", () => {
    test("should match simple patterns", () => {
      ...
      expect(matchesPattern("develop", "develop")).toBe(true);
    });

    test("should match complex patterns", () => {
      expect(matchesPattern("feature/PROJ-123-test", "feature/*-test")).toBe(
        true,
      );
      expect(matchesPattern("release/v1.2.3", "release/v*.*.*")).toBe(true);
      expect(matchesPattern("hotfix/2024-01-15", "hotfix/????-??-??")).toBe(
        true,
      );
    });

    test("should handle double asterisk", () => {
      expect(matchesPattern("feature/sub/feature/test", "feature/**")).toBe(
        true,
      );
      expect(matchesPattern("feature/test", "feature/**")).toBe(true);
    ...
    test("should handle negation in filterBranches", () => {
      ...
      const featureNotProd = filterBranches(branches, [
        "feature/*",
        "!feature/prod",
      ]);
  ...
  describe("extractProjectFromBranch", () => {
    test("should extract project from feature branches", () => {
      expect(extractProjectFromBranch("feature/PROJ-123-test")).toBe(
        "PROJ-123",
      );
    ...
    test("should extract project from bugfix branches", () => {
      expect(extractProjectFromBranch("bugfix/PROJ-789-issue")).toBe(
        "PROJ-789",
      );
