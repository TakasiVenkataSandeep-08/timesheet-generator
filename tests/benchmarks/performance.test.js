...
describe('Performance Benchmarks', () => {
  describe('Time Estimation Performance', () => {
    it('should estimate time for large commit sets quickly', () => {
      ...
      const sessions = Array(100).fill(null).map((_, i) => {
        const hour = 9 + (i % 8);
        const startDate = new Date(`2024-01-01T${String(hour).padStart(2, '0')}:00:00Z`);
        const endDate = new Date(`2024-01-01T${String(hour + 1).padStart(2, '0')}:00:00Z`);
        return {
          start: startDate,
          end: endDate,
          commits: Array(5).fill(null).map((_, j) => ({
            hash: `commit-${i}-${j}`,
            date: new Date(`2024-01-01T${String(hour).padStart(2, '0')}:${String(j * 10).padStart(2, '0')}:00Z`),
            message: `Commit ${i}-${j}`,
            fileStats: [
  ...
  describe('Session Grouping Performance', () => {
    it('should group large commit sets quickly', () => {
      ...
      const commits = Array(1000).fill(null).map((_, i) => {
        ...
        return {
          hash: `commit-${i}`,
          date: new Date(`2024-01-01T${String(hour).padStart(2, '0')}:${String(minute).padStart(2, '0')}:00Z`),
          message: `Commit ${i}`,
          branches: ['main'],
        };
      });
  ...
  describe('Timesheet Generation Performance', () => {
    it('should generate timesheet for large commit sets quickly', async () => {
      ...
      expect(timesheet.totalCommits).toBeGreaterThanOrEqual(500);
  ...
  describe('Multi-Repo Performance', () => {
    it('should process multiple repos efficiently', async () => {
      ...
      expect(timesheet.totalCommits).toBeGreaterThanOrEqual(200);
      expect(duration).toBeLessThan(1000); // Should complete in <1s
    });
  });
});
